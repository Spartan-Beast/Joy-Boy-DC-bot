...
class Commands(commands.Cog):
    ...
    async def announce(self, ctx):
        embed = discord.Embed(title='Announce', description='Running this command in a bot-cmd channel will send an announcement message to the server\'s announcements channel. Write your message after the command.', color=ctx.author.color)
        embed.add_field(name='**Syntax**', value=f'{configData["Prefix"]}announce <message>')
        embed.set_footer(icon_url=ctx.author.avatar_url, text=f"Requested by {ctx.author.name}")
        await ctx.send(embed=embed)
